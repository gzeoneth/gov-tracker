...
- **Election cache freezes mid-pipeline** - `trackElection()` short-circuited and returned the cached checkpoint whenever `phase === "COMPLETED"`, but the election phase reaches `COMPLETED` the moment the member proposal executes on L2 — ~10 days before the cross-chain timelock stages (`L2_TO_L1_MESSAGE` → `RETRYABLE_EXECUTED`) finish. Any cached `election --track` run in that window persisted a checkpoint with `phase: COMPLETED` and the timelock stages frozen at `PENDING`/`NOT_STARTED`, and every subsequent cached run returned that stale state with zero RPC calls, never advancing the pipeline. The short-circuit (and `formatElectionResult()`'s `Complete:` line, which had the same `phase`-only flaw) now gates on the new `isStageSetComplete()` helper — the stage set must be non-empty and every stage terminal.
- **`trackVotingStage` crash on freshly-created proposals** - The pre-start gate in `trackVotingStage()` compared `proposalData.startBlock` against the current L2 block, but Arbitrum governors clock proposals in L1 block space. For a proposal created but not yet at `startBlock`, L2 (~454M) always exceeds startBlock (~25M), so the stage fell through to `getVotingData()`, whose `governor.quorum(startBlock)` reverts with `"Checkpoints: block not yet mined"` — the multicall returned `undefined` for that slot and `BigNumber.from(undefined)` crashed tracking entirely. The gate now fetches the current L1 block via `getL1BlockNumberFromL2()` and compares in L1 space; the pending-ETA calculation also uses `BLOCK_TIMES.L1`. Same fix applied to the `Active` branch's remaining-seconds calculation for accurate ETAs.
...
- **Age gate anchors to on-chain time** - `queryIncompleteCheckpoints()` `maxAgeDays` gate now compares against the immutable timestamp of the checkpoint's first stage (proposal creation block / timelock schedule block) via the new `getCheckpointAnchorTime()` helper, falling back to `checkpoint.createdAt` only when no stage timestamp is available. Previously the gate measured cache-write age, which `saveModularCheckpoints` refreshes on every save — so proposals whose on-chain creation was years old but whose cache entries were rewritten yesterday never aged out.
- **`currentState` reflects lifecycle progression** - `getProposalState()` and `extractProposals()` now derive the OZ Governor `ProposalState` from the stage set via the new `deriveProposalState()` helper instead of reading the `VOTING_ACTIVE.proposalState` snapshot. The snapshot was captured once when voting ended and then frozen, so a fully-executed proposal still reported `"Queued"` forever. Derivation maps `RETRYABLE_EXECUTED=COMPLETED` (or `L2_TIMELOCK=COMPLETED` with `L2_TO_L1_MESSAGE=SKIPPED` for L2-only flows) to `"Executed"`, cascades through `"Canceled"` / `"Defeated"` / `"Queued"` / `"Succeeded"`, and falls back to the snapshot for pre-queue phases (`"Active"` / `"Pending"`) where it is still accurate.
- **`extractProposals()` merges linked timelock stages** - `extractProposals()` now follows `metadata.timelockOpKey` to the linked timelock checkpoint and merges its stages into the returned `stages` array via `mergeStages` + `normalizeTimeline`. Consumers like tally-zero's list view previously saw only the 3 parent stages and a frozen `"Queued"` state even after the timelock fully executed; they now see the complete 7-stage lifecycle with a derived `"Executed"` state. Against the current bundled cache this flips 69 proposals from `"Queued"` to their correct terminal state.
