...
/**
 * JsonStateWriter class for periodic state output
 */
export class JsonStateWriter {
  private readonly outputPath: string;
  private readonly intervalMs: number;
  private intervalHandle: NodeJS.Timeout | null = null;
  private getCheckpoints: () => Map<string, TrackingCheckpoint>;

  constructor(options: {
    outputPath: string;
    intervalSeconds?: number;
    getCheckpoints: () => Map<string, TrackingCheckpoint>;
  }) {
    this.outputPath = options.outputPath;
    this.intervalMs = (options.intervalSeconds ?? 30) * 1000;
    this.getCheckpoints = options.getCheckpoints;
  }
  ...
  write(): boolean {
    try {
      ...
      writeDashboardState(state, this.outputPath);
      return true;
    } catch (err) {
      console.error(
        `Failed to write dashboard state to ${this.outputPath}:`,
   ...
   */
  start(): void {
    this.write(); // Write immediately (errors are logged by write())
    this.intervalHandle = setInterval(() => this.write(), this.intervalMs);
  }
