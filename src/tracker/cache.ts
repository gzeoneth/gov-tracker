...
/**
 * Bundled cache data type: a map of cache keys to tracking checkpoints.
 * Keys are formatted as `tx:<lowercaseTxHash>`.
 */
export type BundledCacheData = Record<string, TrackingCheckpoint>;

/**
 * Get the bundled cache data shipped with the npm package.
 *
 * Returns the parsed JSON data directly, making it suitable for environments
 * where file paths don't work (e.g., Next.js static exports, edge functions).
 *
 * For Node.js file-based usage, prefer `getBundledCachePath()` which avoids
 * loading the entire cache into memory.
 *
 * @returns Parsed bundled cache data
 * @throws Error if bundled cache is not found
 *
 * @example
 * ```typescript
 ...
 *
 * // Populate cache with bundled checkpoints
 * for (const [key, checkpoint] of Object.entries(bundledData)) {
 *   await cache.set(key, checkpoint);
 * }
 *
 * // Now create tracker with pre-populated cache
 * const tracker = createTracker({ ...providers, cache });
 * ```
 *
 * @example
 * ```typescript
 * // Next.js: Use in getStaticProps to pre-populate client cache
 ...
 * ```
 */
export function getBundledCache(): BundledCacheData {
  ...
  return JSON.parse(content) as BundledCacheData;
}
